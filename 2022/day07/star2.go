...
func explore(dir *Dir, space_needed int) (big_dirs []int) {
	for _, f := range dir.Files {
		switch v := f.(type) {
		case *HardFile:
			//NOOP
		case *Dir:
			big_dirs = append(big_dirs, explore(v, space_needed)...)
		}
	}
	if s := dir.Size(); s > space_needed {
		big_dirs = append(big_dirs, s)
	}
	return
}
...
func main() {
	...
	space_available := 70_000_000 - root.Size()
	space_needed := 30_000_000 - space_available
	fmt.Println(min(explore(root, space_needed)))
}
