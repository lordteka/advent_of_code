defmodule Main do
  ...
  defp parse_packet([_v1, _v2, _v3, id1, id2, id3 | packet]) do
  ...
  defp parse_operator(id, [type | packet]) do
    {length, values} = get_sub_packets([type], packet)

    {length + 1, operation(id, values)}
  end

  defp operation(0, values), do: Enum.sum(values)
  defp operation(1, values), do: Enum.product(values)
  defp operation(2, values), do: Enum.min(values)
  defp operation(3, values), do: Enum.max(values)

  defp operation(5, [second, first]) do
    if first > second do
      1
    else
      0
  ...
  defp operation(6, [second, first]) do
    if first < second do
      1
    else
      0
  ...
  defp operation(7, [second, first]) do
    if first == second do
      1
    else
      0
  ...
  defp get_sub_packets('1', [n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11 | _p] = packet) do
    ...
    Enum.reduce(1..subpacket_nb, {11, []}, fn _n, {index, values} ->
      {length, value} = parse_packet(Enum.slice(packet, index, 1_000_000_000))
